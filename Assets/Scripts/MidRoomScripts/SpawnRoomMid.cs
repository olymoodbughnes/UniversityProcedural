...
public class SpawnRoomMid : MonoBehaviour
{
    ...
    private RoomTemplateMid templates;
    ...
    private void Start()
    {
        ...
        templates = GameObject.FindGameObjectWithTag("MiddleRooms").GetComponent<RoomTemplateMid>();
    ...
    private void Spawn()
    {
        ...
        starting = startRoom.atStartM;
        //calculate height and width
        height = ownRef.y - startRef.y;
        width =  System.Math.Abs(ownRef.x - startRef.x);
        ...
        if (spawned == false)
        {
            ...
            if (doorOrientation == 2)
            {
                ...
                if (starting == true)
                {
                    rand = Random.Range(0, templates.rightRooms4.Length);
                    Instantiate(templates.rightRooms4[rand], transform.position, Quaternion.identity);
                    spawned = true;
                    startRoom.isAtStartM(false);
                    return;
                }
                ...
                if (widthToEnd == 20)
                {
                    ...
                    if (height < 0)
                    {
                        ...
                        Instantiate(templates.leftRooms2[rand], transform.position, Quaternion.identity);
                        spawned = true;
                        return;
                    }
                    ...
                    if (height == 0)
                    {
                        ...
                        rand = Random.Range(0, templates.leftRooms3.Length);
                        Instantiate(templates.leftRooms3[rand], transform.position, Quaternion.identity);
                        spawned = true;
                        return;
                    }
                }
    ...
    {
        if (otherRoom.CompareTag("SpawnPointMid") && otherRoom.GetComponent<SpawnRoomMid>().spawned == true)
        {
            Destroy(gameObject);
        }
